...
// ─── Transformações do formulário (fora do componente — sem deps) ─────────────
...
const transformDataForLoad = (apiResponse: any) => {
  ...
  return {
    ...
    is_active: isActiveFlag(data.is_active),
  };
};
...
// ─── Steps (fora do componente — sem deps dinâmicas) ─────────────────────────
    ...
    fields: [
      ...
      { field: 'account_number', label: 'Número da Conta',     type: 'text',   placeholder: 'Ex: 12345-6' },
      {
        ...
        render: (value: any, _: any, onChange: any) => {
          const isChecked = isActiveFlag(value);
          return (
            <div className="mt-2">
...
export default function InstituicoesFinanceirasPage() {
  ...
  const [institutions, setInstitutions] = useState<any[]>([]); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const handleDelete = useCallback(
    ...
    [showPopup, showMessage, selectedId, closeForm, fetchInstitutions],
  );
              ...
              {displayedInstitutions.length === 0 ? (
                ...
                displayedInstitutions.map((inst) => (
                  ...
                  >
                    ...
                    <div className={`flex items-center gap-1 flex-shrink-0 ml-2 transition-opacity ${selectedId === inst.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      <button
            ...
            {formMode === 'IDLE' ? (
              <div className="flex items-center justify-center h-full border border-dashed border-ui-border rounded-xl bg-surface-subtle text-content-muted p-6 text-center m-4">
                <p className="text-[15px]">
                  Selecione uma instituição na lista ao lado para editar seus detalhes ou clique em{' '}
                  <strong>+</strong> para criar uma nova.
                </p>
              </div>
            ) : (
