...
function isMonetaryValue(value: string | number): boolean {
  return typeof value === 'string' && (value.includes('R$') || value.includes('$'));
}

function extractNumericValue(value: string | number): number {
  if (typeof value === 'number') return value;
  const cleaned = value.replace('R$', '').replace(/\./g, '').replace(',', '.').replace(/\s+/g, '').trim();
  const num = parseFloat(cleaned);
  return isNaN(num) ? 0 : num;
}

// ─── Variation Arrow SVGs ────────────────────────────────────────────────────
...
export default function NumericCard({
  ...
  const valueInfo = useMemo(() => {
    if (isMonetaryValue(value)) {
      return { numericValue: extractNumericValue(value), isMonetary: true, isNumeric: true };
    }
    const numericValue = typeof value === 'string' ? parseFloat(value.replace(',', '.')) : value;
    const isNumeric = !isNaN(numericValue);
    return { numericValue: isNumeric ? numericValue : 0, isMonetary: false, isNumeric };
  }, [value]);

  // ⚠️ All hooks must be declared before any early return
  useEffect(() => {
  ...
  const renderMainValue = useCallback(() => {
    ...
    if (valueInfo.isNumeric) {
      return (
        <CountUp
          ...
          separator="."
          decimal=","
          decimals={Number.isInteger(valueInfo.numericValue) ? 0 : 2}
        />
      );
    }
    return value;
    ...
    return (
      <div className="p-4 bg-surface rounded-lg shadow-chart border border-ui-border-strong flex flex-col justify-start items-start relative animate-pulse flex-grow min-w-[300px]">
        <div className="text-sm w-full">
          <div className="h-5 bg-surface-subtle rounded w-2/5 mb-2" />
        </div>
        <div className="h-7 bg-surface-subtle rounded w-1/2 mb-2" />
        <div className="absolute bg-surface-subtle rounded-xl px-1 shadow-chart w-[70px] h-[25px] left-[70%] top-[50px] animate-pulse" />
        {data && <div className="mt-2 h-12 w-full bg-surface-subtle rounded" />}
      </div>
    );
  }
    ...
    <div className="p-4 bg-surface rounded-lg shadow-chart border border-ui-border-strong flex flex-col justify-start items-start relative group hover:shadow-lg transition-all duration-300 flex-grow min-w-[300px]">
      ...
      {variation !== undefined && (
        <div className="absolute bg-surface-muted rounded-xl px-1 shadow-chart w-[70px] h-[25px] flex items-center justify-center gap-2 left-[70%] top-[50px]">
          ...
          {positive === true && <ArrowUp />}
          {positive === false && <ArrowDown />}
          {positive === 'equal' && <ArrowEqual />}
        </div>
      )}

      <div className="text-2xl font-bold text-content">{renderMainValue()}</div>
