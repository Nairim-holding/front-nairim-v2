...
const formatMoney = (value: any) => {
  ...
  const num = typeof value === 'string' ? parseFloat(value.replace(/\D/g, '')) / 100 : Number(value);
...
const maskMoneyInput = (value: string | number) => {
            ...
            <div className="p-6 overflow-y-auto space-y-5">
              ...
              {tempIptu.payment_condition === 'IN_FULL_15_DISCOUNT' && (
                <div>
                  ...
                  <input type="text" value={maskMoneyInput(Number(tempIptu.property_tax_cash) * 100)} onChange={e => setTempIptu({...tempIptu, property_tax_cash: unmaskMoney(e.target.value)})} className="w-full p-2.5 border rounded-lg outline-none focus:border-brand text-sm" placeholder="R$ 0,00" />
                </div>
              )}
              ...
              {tempIptu.payment_condition === 'SECOND_INSTALLMENT_10_DISCOUNT' && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={maskMoneyInput(Number(tempIptu.property_tax_first_installment) * 100)} onChange={e => setTempIptu({...tempIptu, property_tax_first_installment: unmaskMoney(e.target.value)})} className="w-full p-2.5 border rounded-lg outline-none focus:border-brand text-sm" placeholder="R$ 0,00" />
                  </div>
                  <div>
                    <label className="text-xs font-bold mb-1.5 block text-content-secondary">2ª Parcela</label>
                    <input type="text" value={maskMoneyInput(Number(tempIptu.property_tax_second_installment) * 100)} onChange={e => setTempIptu({...tempIptu, property_tax_second_installment: unmaskMoney(e.target.value)})} className="w-full p-2.5 border rounded-lg outline-none focus:border-brand text-sm" placeholder="R$ 0,00" />
                  </div>
                </div>
              )}
              ...
              {tempIptu.payment_condition === 'INSTALLMENTS' && (
                <div>
                  ...
                  <input type="number" value={tempIptu.iptu_installments_count || ''} onChange={e => generateInstallments(e.target.value)} className="w-full p-2.5 border rounded-lg outline-none focus:border-brand text-sm mb-4" />
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-48 overflow-y-auto p-1 custom-scrollbar">
                    {tempIptu.iptu_installments?.map((inst, i) => (
                      <div key={i} className="p-3 border rounded-lg space-y-2 bg-surface-subtle">
                        ...
                        <input type="text" value={maskMoneyInput(Number(inst.value) * 100)} onChange={e => { const n = [...tempIptu.iptu_installments!]; n[i].value = unmaskMoney(e.target.value); setTempIptu({...tempIptu, iptu_installments: n}) }} className="text-xs p-2 border rounded w-full font-bold outline-none focus:border-brand" />
                      </div>
                    ))}
                  </div>
                </div>
