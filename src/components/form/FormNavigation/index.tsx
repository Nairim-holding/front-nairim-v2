...
export default function NavigationButtons({
  ...
  const isSubmitDisabled = loading || formComplete;
    ...
    <div className="flex items-center gap-5 mt-3 border-t-2 pt-3 border-ui-border w-full justify-end">
      ...
      {submitButton && (
        <button
          ...
          type="submit"
          className={`flex justify-center gap-3 items-center max-w-[250px] w-full h-[50px] bg-gradient-to-r from-brand to-brand-hover rounded-lg text-[16px] font-medium text-content-inverse border border-brand drop-shadow-purple-soft ${isSubmitDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          {svg}
